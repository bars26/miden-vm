...
impl Arbitrary for RangeCheckReplayValues {
    ...
    fn arbitrary_with(_args: Self::Parameters) -> Self::Strategy {
        proptest::prop_oneof![
            any::<[u16; 2]>().prop_map(Self::Two),
            any::<[u16; 4]>().prop_map(Self::Four),
        ]
        .boxed()
    }
}
...
impl Arbitrary for RangeCheckerReplay {
    ...
    fn arbitrary_with(_args: Self::Parameters) -> Self::Strategy {
        arb_vec_deque(any::<RangeCheckReplayValues>())
            .prop_map(|range_checks| Self { range_checks })
            .boxed()
    }
}
...
impl Arbitrary for HasherRequestReplay {
    ...
    fn arbitrary_with(_args: Self::Parameters) -> Self::Strategy {
        arb_vec_deque(any::<HasherOp>())
            .prop_map(|ops| Self { sink: HasherOpSink::Buffered(ops) })
            .boxed()
    }
}
