    ...
    S: PackageCache,
{
    ...
    fn should_cache_linked_kernel_package(&self, package: &MastPackage) -> bool {
        let version = PackageVersion::new(package.version.clone(), package.commitment());
...
fn load_selected_preassembled_package(
    ...
    let actual = PackageVersion::new(package.version.clone(), package.commitment());
    if &actual != selected {
        return Err(Report::msg(format!(
